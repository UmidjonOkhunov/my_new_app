...
class AnimatedRouteExampleState extends State<AnimatedRouteExample>
  ...
  void _addRouteLineLayerAndSource() async {
    ...
    await mapboxMap.style.addSource(GeoJsonSource(id: "source", lineMetrics: true));
  }
  ...
  refreshCarAnnotations() async {
    if (showAnnotations) {
      ...
      final camera = await mapboxMap.cameraForCoordinates(
          [...coordinates.map((e) => e), Point(coordinates: myCoordinate)], defaultEdgeInsets, null, null);
  ...
  drawRouteLowLevel(List<Position> polyline) async {
    ...
    animation = Tween<double>(begin: 0, end: 1.0).animate(controller!)
      ..addListener(() async {
        ...
        mapboxMap.style.setStyleLayerProperty("layer", "line-trim-offset", [animation?.value, 1.0]);
      });
    controller?.forward();
  }
}
