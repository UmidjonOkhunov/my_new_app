...
class PointAnnotationExampleState extends State<PointAnnotationExample> {
  ...
  _onMapCreated(MapboxMap mapboxMap) {
    ...
    mapboxMap.annotations.createPointAnnotationManager().then((value) async {
      ...
      for (var i = 0; i < 5; i++) {
        options.add(PointAnnotationOptions(geometry: createRandomPoint(), image: list));
      }
      pointAnnotationManager?.createMulti(options);

      var carOptions = <PointAnnotationOptions>[];
      for (var i = 0; i < 20; i++) {
        carOptions.add(PointAnnotationOptions(geometry: createRandomPoint(), iconImage: "car-15"));
      }
