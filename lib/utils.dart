...
Position createRandomPosition() {
  var random = Random();
  return Position(random.nextDouble() * -360.0 + 180.0, random.nextDouble() * -180.0 + 90.0);
}
...
int createRandomColor() {
  var random = Random();
  return Color.fromARGB(255, random.nextInt(255), random.nextInt(255), random.nextInt(255)).value;
}
...
Position createRandomPositionAround(Position myPosition) {
  var random = Random();
  return Position(myPosition.lng + random.nextDouble() / 10, myPosition.lat + random.nextDouble() / 10);
}
...
extension PolylineCreation on PolylineAnnotationManager {
  addAnnotation(List<Position> coordinates) {
    return PolylineAnnotationOptions(
        geometry: LineString(coordinates: coordinates), lineColor: Colors.red.value, lineWidth: 2);
  }
}

Future<List<Position>> fetchRouteCoordinates(Position start, Position end, String accessToken) async {
...
Future<http.Response> fetchDirectionRoute(Position start, Position end, String accessToken) async {
  final uri = Uri.parse(
